...
class Linear(Module):
    def __init__(self, input_dim, output_dim, init_type="normal", bias=True):
        ...
        if self.init_type == "normal":
            self._parameters["weight"] = np.random.randn(self.input_dim, self.output_dim) * 0.01
            self._parameters["bias"] = np.random.randn(1, self.output_dim) if self.include_bias else None
        elif self.init_type == "zeros":
            self._parameters["weight"] = np.zeros((self.input_dim, self.output_dim))
            self._parameters["bias"] = np.zeros((1, self.output_dim)) if self.include_bias else None
        else:
            raise ValueError(f"Unknown initialization type: {self.init_type}")

        self._gradient["weight"] = np.zeros_like(self._parameters["weight"])
        self._gradient["bias"] = np.zeros_like(self._parameters["bias"]) if self._parameters["bias"] is not None else None
